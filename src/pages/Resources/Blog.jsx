        ...
        <div className="mt-24 grid md:grid-cols-2 gap-8">
          {posts.map((post, i) => (
            <FadeUp key={i} delay={i * 0.1}>
              <article className="border border-[var(--border)] rounded-xl p-8 bg-[var(--bg-card)] hover:border-[var(--accent)] transition">
                <p className="text-xs text-[var(--text-secondary)]">
                  {post.date}
                </p>
                <h3 className="mt-3 text-xl font-semibold text-[var(--accent)]">
                  {post.title}
                </h3>
